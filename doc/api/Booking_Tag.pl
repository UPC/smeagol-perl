[   {   title => 'CreaTag t1',
    ...
    {   title => 'Crea Event simposi',
        ...
        input => {
            description => 'simposi',
            info        => 'simposi info',
            starts      => '2012-04-01T08:00:00Z',
            ends        => '2012-04-10T14:00:00Z',
        },
        status  => sub { shift->code == HTTP_CREATED },
        new_ids => 1,
        output  => [],
    ...
    {
        title => 'Creo un booking per a aula i simposi',
        uri   => '/booking',
        op    => 'POST',
        input => { 'info' => 'info booking',
            'id_resource' => sub { get_generated_id('resource'); },
            'id_event' => sub { get_generated_id('event'); },
            'dtstart' => '2012-04-01T08:00:00Z',
            'dtend' => '2012-04-01T10:00:00Z',
        },
        status => sub { shift->code == HTTP_CREATED },
        new_ids = 1,
        output = [],
...
#        uri   => [ '/resource/', sub { get_generated_id('resource'); }, '/tag' ],
#        op    => 'GET',
...
#        output => [],
#    },
#    {   title => 'Intenta assignar al recurs aula un tag inexistent',
#        uri   => [ '/resource/', \&get_generated_id, '/tag' ],
#        id    => 'dummy',
#        input => {},
#        op    => 'PUT',
...
#        output => [],
#    },
#    {   title => 'Intenta treure-li un tag no assignat al recurs aula',
#        uri   => [ '/resource/', \&get_generated_id, '/tag' ],
#        id    => 'dummy',
#        op    => 'DELETE',
#        status => sub { shift->code == HTTP_NOT_FOUND },
#        output => [],
#    },
#    {   title => 'Treu el tag t2 al recurs aula',
#        uri   => [ '/resource/', \&get_generated_id, '/tag' ],
#        id    => 't2',
#        op    => 'DELETE',
#        status => sub { shift->code == HTTP_OK },
...
#    },
#    {   title =>
#            'Treu el tag t2 al recurs aula (que ara ja no el té assignat)',
#        uri => [ '/resource/', \&get_generated_id, '/tag' ],
#        id  => 't2',
#        op  => 'DELETE',
#        status => sub { shift->code == HTTP_NOT_FOUND },
