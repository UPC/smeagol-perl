[   {   title => 'CreaTag t1',
    ...
    {   title => 'Comprova que el booking NO té assignat el tag t2',
        uri   => [ '/booking/', sub { get_generated_id('booking'); }, '/tag/dummy' ],
        op    => 'GET',
    ...
    {   title => 'Comprova que el booking té assignat el tag t2',
        uri   => [ '/booking/', sub { get_generated_id('booking'); }, '/tag/t2' ],
        op    => 'GET',
        status => sub { shift->code == HTTP_OK },
        output => [],
    },
    {   title =>
            'Comprova que el booking NO té assignat un tag inexistent',
        uri => [ '/booking/', sub { get_generated_id('booking'); }, '/tag/dummy' ],
        op  => 'GET',
        status => sub { shift->code == HTTP_NOT_FOUND },
        output => [],
    ...
    {   title =>
            'Treu el tag t2 al booking (que ara ja no el té assignat)',
        uri => [ '/booking/', sub { get_generated_id('booking'); }, '/tag' ],
        id  => 't2',
        op  => 'DELETE',
        status => sub { shift->code == HTTP_NOT_FOUND },
