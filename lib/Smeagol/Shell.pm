...
sub run_llista_recursos {
    ...
    if ( defined $client ) {
        ...
        print
            "ERROR: No es poden llistar els recursos, no hi ha connexió amb cap servidor smeagol\n";
    }
}
...
sub help_llista_recursos {
    "Abans de poder llistar els recursos, cal que s'hagi connectat a un servidor smeagol previament (veure comanda connecta)\n";
}
...
sub run_crea_recurs {
    ...
    if ( defined $client ) {
        ...
        print
            "ERROR: No es pot crear un recurs, no hi ha connexió amb cap servidor smeagol\n";
    }
}

sub smry_crea_recurs {"Crea un recurs senzill amb una descripció"}

sub help_crea_recurs {
    "Abans de poder crear un recurs, cal que s'hagi connectat a un servidor smeagol previament (veure comanda connecta)\nLa descripció és obligatoria\n";
}
...
sub run_tria_recurs {
    ...
    if ( defined $client ) {
        if ( !defined $id ) {
            ...
            if ( !defined $res ) {
                print
                    "ERROR: Identificador incorrecte. Recurs amb identificador $id no seleccionat\n";
                if ( defined $idResource ) {
                    print
                        "       De moment queda triat el recurs $idResource\n";
                }
                else {
        ...
        print
            "ERROR: No es pot triar un recurs, no hi ha connexió amb cap servidor smeagol\n";
    }
}
...
sub run_mostra_recurs {
    ...
    if ( !defined $idResource ) {
		...
		(defined $res->{agenda})? print "Agenda: ".$res->{agenda}."\n" : print "Agenda:\n";
...
sub help_mostra_recurs {
    "Abans de poder mostrar un recurs, cal que aquest hagi estat triat previament (amb la comanda tria_recurs \"identificador\")\n";
}
...
sub help_esborra_recurs {
    "Abans de poder esborrar un recurs, cal que aquest hagi estat triat previament (amb la comanda tria_recurs \"identificador\")\n";
}
...
sub help_afegeix_etiqueta {
    "Abans de poder afegir una etiqueta a un recurs, cal que aquest hagi estat triat previament (veure comanda tria_recurs \"identificador\"). Una etiqueta ha de tenir entre 2 i 60 caràcters i només pot contenir lletres, números, '.', ':', '_' i '-' \n";
}
...
sub run_esborra_etiqueta {
    ...
    if ( !defined $idResource ) {
        ...
        if ( defined $res ) {
            ...
            print
                "ERROR: No s'ha pogut esborrar l'etiqueta $tag correctament\n";
        }
    }
    else {
        print "ERROR: No hi ha cap etiqueta introduïda.\n";
...
sub help_esborra_etiqueta {
    "Abans de poder esborrar una etiqueta a un recurs, cal que aquest hagi estat triat previament (veure comanda tria_recurs \"identificador\")\n Per esborrar una etiqueta cal introduir el nom d'aquesta\n";
}
...
sub help_mostra_etiquetes {
    "Abans de poder mostrar les etiquetes d'un recurs, cal que aquest hagi estat triat previament (amb la comanda tria_recurs \"identificador\")\n";
}
    ...
    if ( !defined $idResource ) {
        ...
        if ( $f =~ /(\d+)\-(\d+)\-(\d+)\-(\d+):(\d+)/ ) {
            ...
            if ( $t =~ /(\d+)\-(\d+)\-(\d+)\-(\d+):(\d+)/ ) {
                ...
                $to->{minute} = $5;
                $to->{second} = 00;
                my $res = $client->createBooking( $idResource, $desc, $from,
                    $to );
                ...
                if ( defined $res ) {
                    ...
                    print
                        "ERROR: No s'ha pogut crear la reserva correctament\n";
                }
            }
            else {
                print
                    "ERROR: Moment de fi amb format incorrecte (veure ajuda comanda crea_reserva)\n";
            }
        }
        else {
            print
                "ERROR: Moment d'inici amb format incorrecte (veure ajuda comanda crea_reserva)\n";
        }
    }
    else {
        print
            "ERROR: les dades no han estat bé introduides (veure ajuda comanda crea_reserva).\n";
    }
}

sub smry_crea_reserva {"Crea una reserva pel recurs escollit"}

sub help_crea_reserva {
    "Abans de poder crear una reserva per un recurs, cal que aquest hagi estat triat previament (veure comanda tria_recurs \"identificador\")\nPer esborrar una etiqueta cal introduir una descripcio, un moment d'inici (aaaa-mm-dd-hh:mimi), i un altre de fi\n\tp.e. \"presentació tesis\" 2007-12-01-20:00   2007-12-01-21:00\n";
}
...
sub run_esborra_reserva {
    ...
    if ( !defined $idResource ) {
        ...
        if ( defined $res ) {
            ...
            print
                "ERROR: No s'ha pogut esborrar la reserva $id correctament\n";
        }
    }
    else {
...
sub help_esborra_reserva {
    "Abans de poder esborrar una reserva d'un recurs, cal que aquest hagi estat triat previament (veure comanda tria_recurs \"identificador\")\nPer esborrar una reserva cal introduir l'identificador d'aquesta\n";
}
...
sub help_mostra_reserva {
    "Abans de poder mostrar una reserva d'un recurs, cal que aquest hagi estat triat previament (amb la comanda tria_recurs \"identificador\")\nPer mostrar una reserva cal introduir l'identificador d'aquesta\n";
}
...
sub help_mostra_reserves {
    "Abans de poder mostrar les reserves d'un recurs, cal que aquest hagi estat triat previament (amb la comanda tria_recurs \"identificador\")\n";
}
