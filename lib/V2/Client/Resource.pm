...
sub list {
    ...
    foreach (@$perl_scalar) {
        if ( defined $$perl_scalar[0] ) {
            my $scr = V2::Client::Resource->new( url => $self->url );
...
sub create {
    my $self = shift;
    my %args = @_;
    my ( $description, $info, $tags )
        = ( $args{desc}, $args{info}, $args{tags} );
    return unless defined $description;

    my $res = $self->ua->request( POST $self->url . '/resource',
        [ description => $description, info => $info, tags => $tags ] );
    my $perl_scalar = from_json( $res->content, { utf8 => 1 } );
...
#FIXME: Si no se modifican los tags, $res->tags contiene un array de objetos de tipo V2::Client::Tag

    my $req = HTTP::Request->new( PUT $self->url . $id );

#    my $res = $self->ua->request( PUT  $self->url.$id, [ description => $desc, info =>  $info, tags => $tags]);
    $res = $self->ua->request( $req,
        [ description => $desc, info => $info, tags => $tags ] );
