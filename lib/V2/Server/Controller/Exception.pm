...
#booking aren't provided
    ...
    my $boo = $c->model('DB::TBooking')->find({ id => $new_exception->id_booking });
    my $boo_ok;
    ...
    if (1) {
        ...
        my @message
            = { message => "Error: Check if the booking exist",
            };
        $c->stash->{content} = \@message;
        $c->response->status(400);
        $c->stash->{error}
            = "Error: Check if the booking exist";
        $c->stash->{template} = 'exception/get_list.tt';
...
#booking aren't provided
    ...
    $c->visit('/check/check_exception',[]);
    my @message;
    if ( $c->model('DB::TBooking')->find({ id => $exception_aux->id_booking })) {
...
sub default_DELETE {
	...
	my @message = { message => "We have not found the exception. Maybe it's already deleted"};
	$c->stash->{content} = \@message;
