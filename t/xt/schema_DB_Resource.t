...
unless (my $resource_edit = $c->model('DB::Resource')->find({id=>$id_new})){
...
diag("\nresource info: ".$resource_check->info." and should be ".'Test (edited)');
diag("Info edition failed") unless $resource_check->info eq 'Test (edited)';

diag("\nresource info: ".$resource_check->description." and should be ".'Test resource (edited)');
diag("Description edition failed") unless $resource_check->description eq 'Test resource (edited)';
...
diag("############################################################## \n");
  ...
  unless (my $resource_check = $c->model('DB::Resource')->find({id=>$id_new})) {
    diag("Delete test ok \n");
  }else{
