...
diag(     "\nId resource is: "
        . $check_booking->id_resource->id
        . " and should be 2" );
diag("id_resource edition failed")
    unless $check_booking->id_resource->id eq '2';

diag( "\nId event is: " . $check_booking->id_event->id . " and should be 3" );
diag("id_event edition failed") unless $check_booking->id_event->id eq '3';

diag(     "\nstarts is: "
        . $check_booking->starts->iso8601()
        . " and should be 2010-06-16T07:00:00" );
diag("starts edition failed")
    unless $check_booking->starts->iso8601() eq '2010-06-16T05:00:00';

diag(     "\nends is: "
        . $check_booking->ends->iso8601()
        . " and should be 2010-06-16T05:00:00" );
diag("ends edition failed")
    unless $check_booking->ends->iso8601() eq '2010-06-16T07:00:00';
...
unless ( my $check_booking2
    = $c->model('DB::Booking')->find( { id => $id_test } ) )
{
    diag("Delete test ok \n");
}
else {
